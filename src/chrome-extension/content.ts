...
function getCountryOfOrigin(): string | null {
  ...
  const detailsElements = document.querySelectorAll(
    "#detailBullets_feature_div li, #productDetails_detailBullets_sections1 tr, #prodDetails .prodDetSectionEntry"
  );

  for (const element of detailsElements) {
    const text = element.textContent || "";
    if (
      text.toLowerCase().includes("country of origin") ||
      text.toLowerCase().includes("origin country")
    ) {
      // Extract country name
      const match =
        text.match(/origin\s*[:\s]\s*([^,\n]+)/i) ||
  ...
  const techDetails = document.querySelectorAll(
    ".techDetailsList .techDetailsListItem"
  );
...
function createTariffElement(
  ...
  container.style.cssText =
    "margin: 10px 0; padding: 10px; border: 1px solid #e7e7e7; border-radius: 4px; background-color: #f8f8f8;";
...
// Create a skeleton loader for the tariff UI
function createTariffSkeleton(): HTMLElement {
  const container = document.createElement("div");
  container.className = "amazon-tariff-info";
  container.style.cssText =
    "margin: 10px 0; padding: 10px; border: 1px solid #e7e7e7; border-radius: 4px; background-color: #f8f8f8; min-height: 60px; display: flex; flex-direction: column; gap: 8px;";
  container.innerHTML = `
    <div class="animate-pulse flex flex-col gap-2">
      <div class="h-4 size-32 bg-gray-200 rounded"></div>
      <div class="h-6 size-40 bg-gray-300 rounded"></div>
      <div class="h-3 size-24 bg-gray-100 rounded"></div>
    </div>
  `;
  return container;
}
...
async function injectTariffInfo() {
  ...
  if (title && price !== null) {
    ...
    if (
      ...
      countryOfOrigin.toLowerCase().includes("china")
    ) {
      ...
      if (pricingElement) {
        ...
        const skeleton = createTariffSkeleton();
        pricingElement.parentNode?.insertBefore(
          skeleton,
          pricingElement.nextSibling
        );

        // Calculate tariff and replace skeleton
        const tariffInfo = await calculateTariff(productInfo);
        const tariffElement = createTariffElement(price, tariffInfo);
        skeleton.replaceWith(tariffElement);
      }
    }
  }
}
